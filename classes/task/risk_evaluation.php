...
class risk_evaluation extends \core\task\scheduled_task {
    ...
    public function execute(): void {
        ...
        // Fail fast if the engine is unreachable — otherwise this task would
        // spend one connect-timeout per course before giving up on each.
        $health = local_saipa_engine_request('/health', null, 10);
        if (isset($health['error'])) {
            mtrace('SAIPA: engine unreachable (' . $health['error'] .
                   ') — aborting nightly risk evaluation, will retry on the next run.');
            return;
        }
