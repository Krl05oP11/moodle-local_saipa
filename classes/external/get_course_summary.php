...
class get_course_summary extends external_api {
    ...
    public static function execute(int $course_id): array {
        ...
        $adoption_rate = $enrolled > 0 ? round($active_users / $enrolled, 4) : 0.0;
        ...
        $positive_feedback = (int) ($fb->pos ?? 0);
        $negative_feedback = (int) ($fb->neg ?? 0);
        $fb_total          = $positive_feedback + $negative_feedback;
        $feedback_ratio    = $fb_total > 0 ? round($positive_feedback / $fb_total, 4) : -1.0;
        ...
        $high_risk_count   = (int) ($risk->high_cnt ?? 0);
        $medium_risk_count = (int) ($risk->med_cnt ?? 0);
        $low_risk_count    = (int) ($risk->low_cnt ?? 0);
        $last_risk_computed = (int) ($risk->last_computed ?? 0);
        ...
        $chunk_count   = $idx ? (int) $idx->chunk_count : 0;

        // ── Alerts last 30 days ───────────────────────────────────────────────
        $since_30d = time() - (30 * 86400);
        $alerts = $DB->get_record_sql(
            ...
            ['cid' => $cid, 'since' => $since_30d]
        );
        $alerts_sent_30d      = (int) ($alerts->sent ?? 0);
        $alerts_responded_30d = (int) ($alerts->responded ?? 0);
        $alert_response_rate  = $alerts_sent_30d > 0
            ? round($alerts_responded_30d / $alerts_sent_30d, 4) : 0.0;

        return [
            'enrolled_students'     => $enrolled,
            'active_saipa_users'    => $active_users,
            ...
            'last_risk_computed'    => $last_risk_computed,
            'index_status'          => $index_status,
            'last_indexed'          => $last_indexed,
            'chunk_count'           => $chunk_count,
            'alerts_sent_30d'       => $alerts_sent_30d,
            'alerts_responded_30d'  => $alerts_responded_30d,
