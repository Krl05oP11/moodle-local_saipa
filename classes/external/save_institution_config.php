...
class save_institution_config extends external_api {
    public static function execute_parameters(): external_function_parameters {
        return new external_function_parameters([
            'risk_threshold_medium'  => new external_value(PARAM_FLOAT, 'Medium risk threshold (0-1)',         VALUE_OPTIONAL),
            'risk_threshold_high'    => new external_value(PARAM_FLOAT, 'High risk threshold (0-1)',           VALUE_OPTIONAL),
            'alert_cooldown_hours'   => new external_value(PARAM_INT,   'Min hours between alerts to same student', VALUE_OPTIONAL),
            'data_retention_days'    => new external_value(PARAM_INT,   'Days to keep messages/scores (GDPR)', VALUE_OPTIONAL),
            'default_alert_template' => new external_value(PARAM_TEXT,  'Default alert message template',     VALUE_OPTIONAL),
            'risk_eval_enabled'      => new external_value(PARAM_BOOL,  'Risk evaluation globally on/off',    VALUE_OPTIONAL),
            'rag_global_enabled'     => new external_value(PARAM_BOOL,  'RAG globally on/off',                VALUE_OPTIONAL),
        ]);
    }

    public static function execute(
        ?float  $risk_threshold_medium  = null,
        ?float  $risk_threshold_high    = null,
        ?int    $alert_cooldown_hours   = null,
        ?int    $data_retention_days    = null,
        ?string $default_alert_template = null,
        ?bool   $risk_eval_enabled      = null,
        ?bool   $rag_global_enabled     = null
    ): array {
        ...
        // Validate thresholds are in [0, 1] range and medium < high.
        if ($params['risk_threshold_medium'] !== null) {
            $med = (float) $params['risk_threshold_medium'];
            if ($med < 0.0 || $med > 1.0) {
                return ['success' => false, 'message' => 'risk_threshold_medium must be between 0 and 1'];
            }
        }
        if ($params['risk_threshold_high'] !== null) {
            $high = (float) $params['risk_threshold_high'];
            if ($high < 0.0 || $high > 1.0) {
                return ['success' => false, 'message' => 'risk_threshold_high must be between 0 and 1'];
            }
        }
        ...
        foreach ($map as $param_key => $config_key) {
            if ($params[$param_key] !== null) {
                set_config($config_key, $params[$param_key], 'local_saipa');
            }
        }
