...
class get_course_risk extends external_api {
    /**
     * Predefined demo profiles — bypass the engine to guarantee a visible spread
     * of 🟢 low / 🟡 medium / 🔴 high risk. The `risk_level` label is derived from
     * the score and the site's configured thresholds (see demo_result_for), so
     * demo mode always matches what real mode would show.
    ...
    private static function demo_result_for(int $index, int $uid): array {
        ...
        return [
            'userid'     => $uid,
            'score'      => $p['score'],
            'risk_level' => local_saipa_risk_level((float) $p['score']),
    ...
    public static function execute(int $courseid, bool $demo = false): array {
        ...
        [$thresholdmedium, $thresholdhigh] = local_saipa_risk_thresholds();
