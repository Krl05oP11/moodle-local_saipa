...
- Predicts student dropout probability using an XGBoost model trained on 11 engagement features (login frequency, activity completion rate, forum participation, assignment submission ratio, etc.)
...
| **Engine token** | Bearer token configured in the engine (`.env` → `ENGINE_SECRET`) |
